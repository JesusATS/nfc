import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:nfc/providers/tarea_provider.dart';
import 'package:nfc/widgets/common/app_button.dart';
import 'package:nfc/widgets/common/app_dialogs.dart';

class TareaEditScreen extends ConsumerStatefulWidget {
  const TareaEditScreen({Key? key, required this.tareaId}) : super(key: key);
  final String tareaId;

  @override
  ConsumerState<TareaEditScreen> createState() => _TareaEditScreenState();
}

class _TareaEditScreenState extends ConsumerState<TareaEditScreen> {
  final _formKey = GlobalKey<FormState>();
  final _tituloController = TextEditingController();
  final _puntosController = TextEditingController();
  final _iconoController = TextEditingController();

  @override
  void initState() {
    super.initState();
    ref.read(tareaDetailProvider(widget.tareaId).future).then((tarea) {
      if (tarea != null) {
        _tituloController.text = tarea['titulo'] as String;
        _puntosController.text = (tarea['puntos_ganados'] as int).toString();
        _iconoController.text = tarea['icono'] as String? ?? '📝';
      }
    });
  }

  @override
  void dispose() {
    _tituloController.dispose();
    _puntosController.dispose();
    _iconoController.dispose();
    super.dispose();
  }

  Future<void> _editarTarea() async {
    if (_formKey.currentState!.validate()) {
      final titulo = _tituloController.text;
      final puntos = int.parse(_puntosController.text);
      final icono = _iconoController.text;

      try {
        final service = ref.read(tareaServiceProvider);
        await service.update(
          id: widget.tareaId,
          titulo: titulo,
          puntosGanados: puntos,
          icono: icono,
        );
        
        // Invalidar el detalle y lista después de actualizar
        ref.invalidate(tareaDetailProvider(widget.tareaId));
        ref.invalidate(tareaListProvider);
        
        if (mounted) {
          SuccessSnackBar.show(context, message: 'Tarea actualizada');
          Navigator.pop(context);
        }
      } catch (e) {
        if (mounted) {
          ErrorDialog.show(context, message: e.toString());
        }
      }
    }
  }

  @override
  Widget build(BuildContext context) {
    final tareaAsync = ref.watch(tareaDetailProvider(widget.tareaId));

    return Scaffold(
      appBar: AppBar(title: const Text('Editar Tarea')),
      body: tareaAsync.when(
        loading: () => const Center(child: CircularProgressIndicator()),
        error: (err, stack) => Center(child: Text('Error: $err')),
        data: (tarea) {
          if (tarea == null) {
            return const Center(child: Text('Tarea no encontrada'));
          }
          return Form(
            key: _formKey,
            child: SingleChildScrollView(
              padding: const EdgeInsets.all(16.0),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.stretch,
                children: [
                  TextFormField(
                    controller: _tituloController,
                    decoration: const InputDecoration(
                      labelText: 'Título',
                      border: OutlineInputBorder(),
                    ),
                    validator: (value) {
                      if (value == null || value.isEmpty) {
                        return 'Por favor, introduce un título';
                      }
                      return null;
                    },
                  ),
                  const SizedBox(height: 16),
                  TextFormField(
                    controller: _puntosController,
                    decoration: const InputDecoration(
                      labelText: 'Puntos',
                      border: OutlineInputBorder(),
                    ),
                    keyboardType: TextInputType.number,
                    validator: (value) {
                      if (value == null || value.isEmpty) {
                        return 'Por favor, introduce los puntos';
                      }
                      if (int.tryParse(value) == null) {
                        return 'Introduce un número válido';
                      }
                      return null;
                    },
                  ),
                  const SizedBox(height: 16),
                  TextFormField(
                    controller: _iconoController,
                    decoration: const InputDecoration(
                      labelText: 'Icono',
                      border: OutlineInputBorder(),
                    ),
                    validator: (value) {
                      if (value == null || value.isEmpty) {
                        return 'Por favor, introduce un icono';
                      }
                      return null;
                    },
                  ),
                  const SizedBox(height: 32),
                  AppButton(label: 'Actualizar Tarea', onPressed: _editarTarea),
                ],
              ),
            ),
          );
        },
      ),
    );
  }
}
